...
int main(int argc, char *argv[]) {
    ...
    @autoreleasepool {
        ...
        main_script = [
                [[NSBundle mainBundle] pathForResource:@"Library/Application Support/com.example.dimark/app/dimark/app"
                                                ofType:@"py"] cStringUsingEncoding:NSUTF8StringEncoding];

        if (main_script == NULL) {
            NSLog(@"Unable to locate dimark main module file");
            exit(-1);
        }
        ...
        @try {
            ...
            if (fd == NULL) {
                ...
                if (ret != 0) {
                    NSLog(@"Application quit abnormally, Python code failed to load!");
                } else {
                    NSLog(@"Python code loaded, trying to get entry point");
